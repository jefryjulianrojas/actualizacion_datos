...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "SELECT proveedor.id_proveedor, proveedor.nombre, proveedor.numero_identificacion, proveedor.contrasena FROM usuarios INNER JOIN proveedor ON usuarios.id_cliente = proveedor.id_cliente WHERE (usuario = ? OR usuario_cliente = ?) AND contrasena = ?";
    ...
    if ($resultado->num_rows > 0) {
        ...
        $_SESSION["proveedor_id"] = $usuario["id_proveedor"];
        $_SESSION["proveedor_nombre"] = $usuario["nombre"];
        $_SESSION["proveedor_numero"] = $usuario["numero_identificacion"];
        $_SESSION["proveedor_contrasena"] = $usuario["contrasena"];
        
        
        if ($_SESSION["proveedor_contrasena"] == $_SESSION["proveedor_numero"]) {
            // El campo está vacío
             // Redirige a la página del formulario
                header("Location: contrasena_pro.php");
             exit();
...
<style>
    ...
    height: 100%;
        width: 100%;    
    background-image: url('images/proveedor.jpg'); /* Ruta de la imagen de fondo */
...
<?php endif; ?>
  ...
  <form method="post" action="<?php echo $_SERVER["PHP_SELF"]; ?>">
    ...
    <input type="submit" value="Iniciar sesión">
      ...
      <br>
            <div class="contene">
      <a href="recuperar_contrasenap1.php" style="text-decoration: none; color: black;">Olvidaste tu contraseña?</a>
      </div>
  </form>
</div>
    </div>
